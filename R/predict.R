...
generate_new_data <- function(season, race, drivers, historical_data = clean_data()) {
  # drivers should be a data.frame or tibble with: driverId, constructorId, (optional any of: quali_position, grid, practice_optimal_rank, practice_best_rank,
  # practice_avg_rank) Any missing race-specific values will be given as current moved averages Any other values will be calculated from historical data,
  # presuming this is next race




  dplyr::select("driverId", "constructorId", "grid", "quali_position", "driver_experience", "driver_failure_avg", "constructor_grid_avg", "constructor_finish_avg",
