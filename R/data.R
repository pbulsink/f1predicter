...
get_laps_or_null <- function(season, round, session) {
  laps <- tryCatch(f1dataR::load_session_laps(season = season, round = round, session = session, add_weather = T), error = function(e) {
    return(NULL)
  })
...
get_grids <- function(season, round, session) {
  if (session == "R") {
    ...
    if (!is.na(schedule[schedule$season == season & schedule$round == round,]$sprint_date)){
  ...
  grid <- data.frame(position = 1:max(nrow(results), nrow(quali))) %>%
    dplyr::mutate(quali_results = expand_val(quali$driver_id, ndrivers, NA),
...
get_weekend_data <- function(season, round, force = FALSE) {
  ...
  if (!force) {
    rgrid <- tryCatch(utils::read.csv(file.path(options("f1predicter.cache"), paste0(season, "_", round, "_rgrid.csv"))), error = function(e) return(NA), warning = function(w) return(NA)) %>%
      ensure_tidy()
    sgrid <- tryCatch(utils::read.csv(file.path(options("f1predicter.cache"), paste0(season, "_", round, "_sgrid.csv"))), error = function(e) return(NA), warning = function(w) return(NA)) %>%
      ensure_tidy()
    results <- tryCatch(utils::read.csv(file.path(options("f1predicter.cache"), paste0(season, "_", round, "_results.csv"))), error = function(e) return(NA), warning = function(w) return(NA)) %>%
      ensure_tidy()
    sprint_results <- tryCatch(utils::read.csv(file.path(options("f1predicter.cache"), paste0(season, "_", round, "_results.csv"))), error = function(e) return(NA), warning = function(w) return(NA)) %>%
      ensure_tidy()
    pitstops <- tryCatch(utils::read.csv(file.path(options("f1predicter.cache"), paste0(season, "_", round, "_pitstops.csv"))), error = function(e) return(NA), warning = function(w) return(NA)) %>%
      ensure_tidy()
    quali <- tryCatch(utils::read.csv(file.path(options("f1predicter.cache"), paste0(season, "_", round, "_quali.csv"))), error = function(e) return(NA), warning = function(w) return(NA)) %>%
      ensure_tidy()
    laps <- tryCatch(utils::read.csv(file.path(options("f1predicter.cache"), paste0(season, "_", round, "_laps.csv")), colClasses = c("character", "character",
    ...
    if (!is.null(results)) {
      ...
      results <- results %>%
        dplyr::mutate(points = as.numeric(.data$points),
                      ...
                      round = round) %>%
        janitor::clean_names()
      utils::write.csv(x = results, file = file.path(options("f1predicter.cache"), paste0(season, "_", round, "_results.csv")), quote = F, row.names = F)
    }
    Sys.sleep(1)
  }
  ...
  if (season >= 2021) {
    if (!any(!is.na(sprint_results))) {
      if(!is.null(schedule[schedule$season == season & schedule$round == round,]$sprint_date)) {
        ...
        if(!is.null(sprint_results)) {
           sprint_results <- sprint_results%>%
            ...
            janitor::clean_names()
          utils::write.csv(x = sprint_results, file = file.path(options("f1predicter.cache"), paste0(season, "_", round, "_sprint_results.csv")), quote = F, row.names = F)
        }
        Sys.sleep(1)
      }
    }
    ...
    if (!any(!is.na(pitstops))) {
      ...
      if (!is.null(pitstops)) {
        if (length(pitstops) == 0) {
          ...
          pitstops <- pitstops %>%
            dplyr::mutate(stop = as.numeric(.data$stop),
                          lap = as.numeric(.data$lap),
                          duration = as.numeric(.data$duration),
                          season = season,
                          round = round) %>%
            janitor::clean_names()
          utils::write.csv(x = pitstops, file = file.path(options("f1predicter.cache"), paste0(season, "_", round, "_pitstops.csv")), quote = F, row.names = F)
        }
      }
      Sys.sleep(1)
    }
    ...
    if (!any(!is.na(rgrid))) {
      ...
      if (!is.null(rgrid)) {
        rgrid <- rgrid %>%
          ...
          janitor::clean_names()
        utils::write.csv(x = rgrid, file = file.path(options("f1predicter.cache"), paste0(season, "_", round, "_rgrid.csv")), quote = F, row.names = F)
      }
      Sys.sleep(1)
    }
    ...
    if (!any(!is.na(sgrid))) {
      ...
      if (!is.null(sgrid)) {
        sgrid <- sgrid %>%
          dplyr::mutate(season = season, round = round) %>%
          janitor::clean_names()
        utils::write.csv(x = sgrid, file = file.path(options("f1predicter.cache"), paste0(season, "_", round, "_sgrid.csv")), quote = F, row.names = F)
      }
      Sys.sleep(1)
    }
    ...
    if (!any(!is.na(quali))) {
      ...
      if (!is.null(quali)) {
        if (length(quali) == 0) {
          ...
          quali <- quali %>%
            dplyr::mutate(position = as.numeric(.data$position), season = season, round = round) %>%
            dplyr::mutate(q1_sec = expand_val(unlist(.data$q1_sec), dplyr::n())) %>%
            janitor::clean_names()
          utils::write.csv(x = quali, file = file.path(options("f1predicter.cache"), paste0(season, "_", round, "_quali.csv")), quote = F, row.names = F)
        }
      }
      Sys.sleep(1)
    }
    ...
    if (!any(!is.na(laps))) {
      ...
      if (!is.null(laps)) {
        laps <- laps %>%
          ...
          dplyr::left_join(results[, c("driver_id", "constructor_id")], by = c("driver_id")) %>%
          dplyr::mutate(season = season, round = round) %>%
          dplyr::select("driver_id", "constructor_id", "lap_time", "lap_number", "stint", "sector1time", "sector2time",
            "sector3time", "speed_i1", "speed_i2", "speed_fl", "speed_st",  "is_personal_best", "compound", "tyre_life",
            "fresh_tyre", "track_status", "position", "deleted", "deleted_reason", "is_accurate", "air_temp", "humidity",
            "pressure", "rainfall", "track_temp", "wind_direction", "wind_speed", "session_type", "season", "round")
        utils::write.csv(x = laps, file = file.path(options("f1predicter.cache"), paste0(season, "_", round, "_laps.csv")), quote = F, row.names = F)
      }
    }
  } else {
    laps = NULL
  ...
  closeAllConnections()
  return(list(rgrid = rgrid, sgrid = sgrid, results = results, sprint_results = sprint_results, pitstops = pitstops, laps = laps, quali = quali))
}
...
get_season_data <- function(season, force = FALSE) {
  ...
  for (round in schedule[schedule$season == season & schedule$date <= Sys.Date(), ]$round) {
    cat("Getting data for round", round, "of", length(schedule[schedule$season == season & schedule$date <= Sys.Date(), ]$round), "\n")
    r <- get_weekend_data(season = season, round = as.numeric(round), force = force)
    rgrid <- dplyr::bind_rows(rgrid, r$rgrid)
    sgrid <- dplyr::bind_rows(sgrid, r$sgrid)
    results <- dplyr::bind_rows(results, r$results)
    sprint_results <- dplyr::bind_rows(results, r$sprint_results)
  ...
  if (season >= 2021){
    sprint_results %>%
      janitor::clean_names() %>%
      utils::write.csv(file = file.path(options("f1predicter.cache"), paste0(season, "_season_sprint_results.csv")), quote = F, row.names = F)
  }
...
load_all_data <- function() {
  ...
  for (y in c(1990:f1dataR::get_current_season())) {
    ...
    if (y >= 2021) {
      sr <- tryCatch(utils::read.csv(file.path(options("f1predicter.cache"), paste0(y, "_season_sprint_results.csv"))), error = function(e) return(NULL)) %>%
        ensure_tidy()
  ...
  return(list(results = results, sprint_results = sprint_results, laps = laps, pitstops = pitstops, sgrid = sgrid, rgrid = rgrid, qualis = qualis))
}
...
getWeather <- function(round_url) {
  ...
  tryCatch({
    ...
    message(glue::glue("Error in f1predictor::getWeather: {url} - {err}", url = round_url, err = e))
  })

  if (is.null(round_weather)) {
    # Try the italian site - it's apparently more robust
    message(glue::glue("f1predictor::getWeather: Trying to get weather from Italian Wikipedia instead of {url}", url = round_url))
    it_url <- grep(pattern = "https:\\/\\/it.wikipedia.org\\/wiki\\/[a-zA-Z0-9_%]*", x = RCurl::getURLContent(round_url, .opts = list(ssl.verifypeer = FALSE)),
      value = TRUE)
  ...
  if (is.null(round_weather)) {
  ...
  } else if (grepl("showers|wet|rain|pioggia|damp|thunderstorms|rainy", round_weather, ignore.case = T)) {
    round_weather <- "wet"
  } else if (grepl("cloudy|grey|coperto|clouds|nuvoloso|overcast", round_weather, ignore.case = T)) {
    ...
    round_weather <- "cold"
  } else if (grepl("soleggiato|clear|warm|hot|sunny|fine|mild|sereno", round_weather, ignore.case = T)) {
    round_weather <- "warm"
  } else {
    message(glue::glue("f1predictor::getWeather: round Weather of {round_weather} is unknown type. From {url}", round_weather = round_weather, url = round_url))
    round_weather <- "unknown"
  }
...
janitor_data <- function(){
  ...
  for (y in c(1990:f1dataR::get_current_season())) {
    ...
    if (y >= 2018) {
      lp <- tryCatch(utils::read.csv(file.path(options("f1predicter.cache"), paste0(y, "_season_laps.csv")), colClasses = c("character", "character", "numeric",                                                                                                   "integer", "integer")), error = function(e) return(NULL)) %>%
        ensure_tidy()
    }
    if (y >= 2021) {
      sr <- tryCatch(utils::read.csv(file.path(options("f1predicter.cache"), paste0(y, "_season_sprint_results.csv")), colClasses = c("character", "character", "numeric",                                                                                                   "integer", "integer")), error = function(e) return(NULL)) %>%
        ensure_tidy()
    }
    closeAllConnections()
    if(!is.null(rg)){
    ...
    if(!is.null(sr)) {
      sr %>%
        janitor::clean_names() %>%
        utils::write.csv(file = file.path(options("f1predicter.cache"), paste0(y, "_season_sprint_results.csv")), quote = F, row.names = F)
    }
    closeAllConnections()
  }
}
