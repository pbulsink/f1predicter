...
get_laps_or_null <- function(season, race, session){
  laps <- tryCatch(
    f1dataR::load_session_laps(season = season, race = race, session = session, add_weather = T),
    error = function(e){return(NULL)}
  )

  if(!is.null(laps)){
    laps <- laps %>%
      dplyr::mutate('IsPersonalBest' = as.logical(unlist(.data$IsPersonalBest)),
                    'TrackStatus' = as.numeric(unlist(.data$TrackStatus)),
                    'DeletedReason' = unlist(.data$DeletedReason),
                    'AirTemp' = unlist(.data$AirTemp),
                    'Humidity' = unlist(.data$Humidity),
                    'Pressure' = unlist(.data$Pressure),
                    'Rainfall' = as.logical(unlist(.data$Rainfall)),
                    'TrackTemp' = unlist(.data$TrackTemp),
                    'WindDirection' = unlist(.data$WindDirection),
                    'WindSpeed' = unlist(.data$WindSpeed))
  }
...
get_grids <- function(season, race, session){
  if(session == 'R'){
    #data frame
    # Position (1-20), QualiResults (Driver), Start Grid (Driver), Final Position (Driver)
  ...
  startgrid <- results %>% dplyr::arrange('grid') %>% dplyr::pull('driverId')
  ndrivers<-max(nrow(results), nrow(quali))
  grid <- data.frame("position" = 1:max(nrow(results), nrow(quali))) %>%
    dplyr::mutate('qualiResults' = expand_va(quali$driverId,ndrivers, NA),
                  'startGrid' = expand_val(startgrid,ndrivers, NA),
                  'raceResults' = expand_val(results$driverId,ndrivers, NA))
  return(grid)
}
...
get_weekend_data <- function(season, race, force = FALSE){
  ...
  if(!force){
    rgrid <- tryCatch(read.csv(file.path(options('f1predicter.cache'), paste0(season, "_", race, "_rgrid.csv"))),
                      error = function(e) return(NA),
                      warning = function(w) return(NA))
    sgrid <- tryCatch(read.csv(file.path(options('f1predicter.cache'), paste0(season, "_", race, "_sgrid.csv"))),
                      error = function(e) return(NA),
                      warning = function(w) return(NA))
    results <- tryCatch(read.csv(file.path(options('f1predicter.cache'), paste0(season, "_", race, "_results.csv"))),
                        error = function(e) return(NA),
                        warning = function(w) return(NA))
    pitstops <- tryCatch(read.csv(file.path(options('f1predicter.cache'), paste0(season, "_", race, "_pitstops.csv"))),
                         error = function(e) return(NA),
                         warning = function(w) return(NA))
    quali <- tryCatch(read.csv(file.path(options('f1predicter.cache'), paste0(season, "_", race, "_quali.csv"))),
                      error = function(e) return(NA),
                      warning = function(w) return(NA))
    laps <- tryCatch(read.csv(file.path(options('f1predicter.cache'), paste0(season, "_", race, "_laps.csv")),
                              colClasses = c('character', 'character', 'numeric', 'integer', 'integer',
                                             ...
                                             'integer', 'integer', 'logical', 'character', 'logical',
                                             'numeric', 'numeric', 'numeric', 'logical', 'numeric',
                                             'integer', 'numeric', 'character', 'integer', 'integer')),
                     error = function(e) return(NA),
                     warning = function(w) return(NA))
    ...
    if(!is.null(results)){
      ...
      results <- results  %>%
        dplyr::mutate('points' = as.numeric(.data$points),
                      ...
                      'season' = season,
                      'race' = race)
      write.csv(x = results, file = file.path(options('f1predicter.cache'), paste0(season, "_", race, "_results.csv")), quote = F, row.names = F)
    }
    Sys.sleep(1)
  }
        ...
        if(length(pitstops) == 0){
          ...
          pitstops <- pitstops  %>%
            dplyr::mutate('stop' = as.numeric(.data$stop),
                          ...
                          'season' = season,
                          'race' = race)
          write.csv(x = pitstops, file = file.path(options('f1predicter.cache'), paste0(season, "_", race, "_pitstops.csv")), quote = F, row.names = F)
        }
      }
      Sys.sleep(1)
    }
  ...
  if(season >= 2003){
    if(!any(!is.na(rgrid))){
      ...
      if(!is.null(rgrid)){
        rgrid <- rgrid  %>% dplyr::mutate('season' = season, 'race' = race)
        write.csv(x = rgrid, file = file.path(options('f1predicter.cache'), paste0(season, "_", race, "_rgrid.csv")), quote = F, row.names = F)
      }
      Sys.sleep(1)
    }
    ...
    if(!any(!is.na(sgrid))){
      ...
      if(!is.null(sgrid)){
        sgrid <- sgrid  %>% dplyr::mutate('season' = season, 'race' = race)
        write.csv(x = sgrid, file = file.path(options('f1predicter.cache'), paste0(season, "_", race, "_sgrid.csv")), quote = F, row.names = F)
      }
      Sys.sleep(1)
    }
    ...
    if(!any(!is.na(quali))){
      ...
      if(!is.null(quali)){
        if(length(quali) == 0){
          ...
          quali <- quali  %>%
            dplyr::mutate('position' = as.numeric(.data$position),
                          'season' = season,
                          'race' = race)
          write.csv(x = quali, file = file.path(options('f1predicter.cache'), paste0(season, "_", race, "_quali.csv")), quote = F, row.names = F)
    ...
    if(!any(!is.na(laps))){
      ...
      if(!is.null(laps)){
        laps <- laps %>%
          ...
          dplyr::select('driverId', 'constructorId', 'lapTime' = 'LapTime', 'lapNumber' = 'LapNumber',
                        'stint' = 'Stint', 'sector1Time' = 'Sector1Time', 'sector2Time' = 'Sector2Time',
                        'sector3Time' = 'Sector3Time', 'speedI1' = 'SpeedI1', 'speedI2' = 'SpeedI2',
                        'speedFL' = 'SpeedFL', 'speedST' = 'SpeedST', 'isPersonalBest' = 'IsPersonalBest',
                        'compound' = 'Compound', 'tireLife' = 'TyreLife', 'freshTire' = 'FreshTyre',
                        'trackStatus' = 'TrackStatus', 'position' = 'Position', 'deleted' = 'Deleted',
                        'deletedReason' = 'DeletedReason', 'isAccurate' = 'IsAccurate', 'airTemp' = 'AirTemp',
                        'humidity' = 'Humidity', 'pressure' = 'Pressure', 'rainfall' = 'Rainfall',
                        'trackTemp' = 'TrackTemp', 'windDirection' = 'WindDirection', 'windSpeed' = 'WindSpeed',
                        'sessionType' = 'SessionType', 'season', 'race')
        write.csv(x = laps, file = file.path(options('f1predicter.cache'), paste0(season, "_", race, "_laps.csv")), quote = F, row.names = F)
...
get_season_data<-function(season, force = FALSE){
  ...
  write.csv(x = rgrid, file = file.path(options('f1predicter.cache'), paste0(season, "_season_rgrid.csv")), quote = F, row.names = F)
  if(!is.null(sgrid)){
    write.csv(x = sgrid, file = file.path(options('f1predicter.cache'), paste0(season, "_season_sgrid.csv")), quote = F, row.names = F)
  }
  if(season >= 2018){
      write.csv(x = laps, file = file.path(options('f1predicter.cache'), paste0(season, "_season_laps.csv")), quote = F, row.names = F)
  }
  write.csv(x = results, file = file.path(options('f1predicter.cache'), paste0(season, "_season_results.csv")), quote = F, row.names = F)
  write.csv(x = pitstops, file = file.path(options('f1predicter.cache'), paste0(season, "_season_pitstops.csv")), quote = F, row.names = F)
  write.csv(x = qualis, file = file.path(options('f1predicter.cache'), paste0(season, "_season_qualis.csv")), quote = F, row.names = F)
  closeAllConnections()
}
...
load_all_data<-function(){
  ...
  for(y in c(1990:f1dataR:::get_current_season())){
    cat("Reading", y, "data...\n")
    rg<-tryCatch(read.csv(file.path(options('f1predicter.cache'), paste0(y, "_season_rgrid.csv"))),
                 error = function(e) return(NULL))
    rgrid<-dplyr::bind_rows(rgrid, rg)
    sg<-tryCatch(read.csv(file.path(options('f1predicter.cache'), paste0(y, "_season_sgrid.csv"))),
                 error = function(e) return(NULL))
    sgrid<-dplyr::bind_rows(sgrid, sg)
    res<-tryCatch(read.csv(file.path(options('f1predicter.cache'), paste0(y, "_season_results.csv"))),
                  error = function(e) return(NULL))
    results<-dplyr::bind_rows(results, res)

    q <- tryCatch(read.csv(file.path(options('f1predicter.cache'), paste0(y, "_season_qualis.csv"))),
                  error = function(e) return(NULL))
    qualis<-dplyr::bind_rows(qualis, q)
    if(y >= 2011){
      pt<-tryCatch(read.csv(file.path(options('f1predicter.cache'), paste0(y, "_season_pitstops.csv"))),
                   error = function(e) return(NULL))
      pitstops<-dplyr::bind_rows(pitstops, pt)
    }
    if(y >= 2018){
      lp<-tryCatch(read.csv(file.path(options('f1predicter.cache'), paste0(y, "_season_laps.csv")),
                            colClasses = c('character', 'character', 'numeric', 'integer', 'integer',
                                           'numeric', 'numeric', 'numeric', 'integer', 'integer', 'integer',
                                           'integer', 'logical', 'character', 'integer', 'logical',
                                           'integer', 'integer', 'logical', 'character', 'logical',
...
get_last_drivers<-function(){
  res<-tryCatch(read.csv(file.path(options('f1predicter.cache'), paste0(f1dataR::get_current_season(), "_season_results.csv"))),
                error = function(e) return(NULL))
