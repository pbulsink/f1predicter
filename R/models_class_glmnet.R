...
model_quali_early_glm <- function(data = clean_data(), cutoff_year = 2018) {
  ...
  pole_mod <- parsnip::logistic_reg(penalty = tune::tune(), mixture = tune::tune()) %>%
    parsnip::set_mode("classification") %>%
    parsnip::set_engine("glmnet", nthread = 4)

  glm_grid <- dials::grid_latin_hypercube(dials::penalty(), dials::mixture())

  pole_wflow <- workflows::workflow() %>%
    workflows::add_model(pole_mod) %>%
    workflows::add_recipe(pole_recipe)

  message("Starting pole model")
  tictoc::tic("Trained Pole Model")
  pole_res <- pole_wflow %>%
    tune::tune_grid(resamples = data_folds, grid = glm_grid, metrics = metrics_binary)
  ...
  message("Starting quali position classification model")
  tictoc::tic("Trained Quali Position Classification Model")
  position_res <- position_wflow %>%
    tune::tune_grid(resamples = data_folds, grid = glm_grid, metrics = metrics_multi)
  ...
  # Note
  position_regress_mod <- parsnip::linear_reg(penalty = tune::tune(), mixture = tune::tune()) %>%
  ...
  position_regress_res <- position_regress_wflow %>%
    tune::tune_grid(resamples = data_folds, grid = glm_grid, metrics = metrics_regression)
...
model_results_after_quali_glm <- function(data = clean_data(), cutoff_year = 2018){
  ...
  message("Win Model with ",
          ...
          round(tune::collect_metrics(win_final_fit) %>% dplyr::filter(.data$.metric == "npv") %>% dplyr::pull(".estimate"), 4), " npv, ",
  ...
  message("Podium Model with ",
          ...
          round(tune::collect_metrics(podium_final_fit) %>% dplyr::filter(.data$.metric == "npv") %>% dplyr::pull(".estimate"), 4), " npv, ",
  ...
  message("T10 Model with ",
          ...
          round(tune::collect_metrics(t10_final_fit) %>% dplyr::filter(.data$.metric == "npv") %>% dplyr::pull(".estimate"), 4), " npv, ",
  ...
  message("Finishing Model with ",
          ...
          round(tune::collect_metrics(finish_final_fit) %>% dplyr::filter(.data$.metric == "npv") %>% dplyr::pull(".estimate"), 4), " npv, ",
