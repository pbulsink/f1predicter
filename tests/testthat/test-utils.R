test_that("nroot() calculates roots correctly", {
...
test_that("cubert() calculates cube roots correctly", {
...
test_that("expand_val() pads vectors correctly", {
...
test_that("cumwmean() calculates cumulative weighted mean correctly", {
  x <- c(1, 2, 3)
...
test_that("ensure_tidy() cleans column names", {
  data <- data.frame(
    "Column One" = 1:3,
    "Column.Two" = 4:6,
    check.names = FALSE
...
test_that("ensure_tidy() renames 'race' to 'round'", {
...
test_that("ensure_tidy() returns NULL if input is NULL", {
  result <- ensure_tidy(NULL)
  expect_null(result)
})

test_that("ensure_tidy() returns NA if input is all NA", {
...
test_that("normalize_vector() sums to 1", {
  x <- c(1, 2, 3, 4)
...
test_that("normalize_vector() handles NA values", {
  x <- c(1, 2, NA, 4)
  result <- normalize_vector(x)

  expect_length(result, 4)
  expect_equal(sum(result, na.rm = T), 1)
  # NA in input should remain NA in output
  expect_true(is.na(result[3]))
})

test_that("normalize_vector() returns uniform distribution for zero sum", {
  x <- c(0, 0, 0)
  result <- normalize_vector(x)
...
test_that("normalize_vector() handles empty vector", {
  result <- normalize_vector(numeric(0))
  expect_length(result, 0)
})

test_that("wmean() returns weighted mean", {
  x <- c(1, 2, 3)
...
test_that("wmean() pads vector with specified value", {
  x <- c(1, 2)
...
test_that("lagged_cumwmean_expanded() lags correctly", {
  x <- c(1, 2, 3)
...
test_that("s_lagged_cumwmean_expanded() returns last value", {
  x <- c(1, 2, 3)
