test_that("prepare_and_split_data() returns required list elements (#noissue)", {
...
test_that("prepare_and_split_data() creates training/test split correctly (#noissue)", {
  test_data <- data.frame(
    round_id = rep(1:5, 4),
...
test_that("prepare_and_split_data() selects specified columns (#noissue)", {
  test_data <- data.frame(
    round_id = 1:5,
    col1 = 1:5,
    col2 = 6:10,
...
test_that("prepare_and_split_data() converts character to factor (#noissue)", {
  test_data <- data.frame(
    round_id = 1:5,
...
test_that("report_model_metrics() handles missing metrics gracefully (#noissue)", {
  # Create a mock last_fit object structure
  # This is a simplified test since we'd need to create a full tune object
  test_df <- data.frame(
  ...
  # Full testing would require mocking tune::collect_metrics
  expect_true(is.function(report_model_metrics))
})
