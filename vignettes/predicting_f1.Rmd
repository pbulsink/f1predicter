...
The prediction models rely on up-to-date historical data. The `get_weekend_data()` function fetches the provided race (season & round) results, qualifying data, pit stops, and lap times and caches them locally as CSV files.
...
The training functions (`model_results_early`, `model_results_late`, `model_results_after_quali`) return `butchered` `parsnip` model objects. Butchering removes the large training data from the model object to reduce file size. These are then saved as `.RDS` files with `save_models()`.
...
The `generate_new_data` function creates a data frame with all the necessary features for the upcoming race. It can optionally take practice and qualifying data to create more accurate predictions after those sessions have occurred.
...
new_race_data <- generate_new_data(
  season = season,
  round_num = round_num
)
```
